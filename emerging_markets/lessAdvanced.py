    ...
    for instrument_id in INSTRUMENT_IDS:
        ...
        if book and book.bids and book.asks:
            ...
            if spread > 0.15:
                ...
                if flag == 1:
                    response: InsertOrderResponse = e.insert_order(instrument_id, price=bid_price, volume=30, side=SIDE_BID, order_type=ORDER_TYPE_LIMIT)
                    print_order_response(response)
                else:
                    response: InsertOrderResponse = e.insert_order(instrument_id, price=ask_price, volume=30, side=SIDE_ASK, order_type=ORDER_TYPE_LIMIT)
                    print_order_response(response)
